...
H1 is directional: the planned hypothesis is a fatigue-related decline/decoupling from Block 1 to Block 5, so frequentist inference uses the planned one-tailed/directional threshold of **alpha = .05**, and Bayesian evidence is reported as an order-restricted directional Bayes factor for `Block 5 < Block 1`.
...
To account for Bayes factor volatility at small sample sizes and protect against premature stopping, we pre-established a minimum sample size of **$N_{min} = 20$**. Data collection is planned to terminate when the directional Bayes factor for H1 exceeds **BF > 3**, up to a resource-constrained maximum of **$N_{max} = 30$**.
